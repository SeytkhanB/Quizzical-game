...
export default function Header(props) {
  const { deleteAll, storeDataState } = useContext(Context);

  const { historyOpenState, openHistoryPage } = useAppLogic();
  ...
  return (
    <nav className={`${props.themeState ? "dark" : ""}`}>
      <div className="logo-container">
        ...
        >
          <p>
            <img width="10px" src={CategoriesIcon} alt="" />{" "}
            {diff.toUpperCase()} {iconOfDifficulty}
          </p>

          <p className="amount-of-questions">
            <img width="10px" src={AmountOfQuestions} alt="" />{" "}
            {props.numPickerState}
          </p>
        </div>
      </div>
      ...
      <div>
        <img
          ...
          src={props.themeState ? HistoryIconWhite : HistoryIconBlack}
          alt="History icon"
      ...
      {historyOpenState && (
        <main className={`history-page ${props.themeState ? "dark" : ""}`}>
          <h1>History</h1>
          <h4 className="delete-all" onClick={deleteAll}>
            Delete all
          </h4>
          {history}
        </main>
